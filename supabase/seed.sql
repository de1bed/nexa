...
select id,id,jsonb_build_object('sub',id,'email',email),'email',email,now(),now(),now() from auth.users where id::text like '10000000-%';
...
select '20000000-0000-0000-0000-000000000001',id,encode(digest(case when id='50000000-0000-0000-0000-000000000001' then 'nexa-demo-invitation-2026' else 'seed-invite-'||id end,'sha256'),'hex'),'••••'||right(id::text,4),ends_at+interval '1 day',now()
...
select organization_id,id,encode(digest(case when id='50000000-0000-0000-0000-000000000003' then 'nexa-demo-pass-2026' else 'seed-pass-'||id end,'sha256'),'hex'),'••••'||right(id::text,4),starts_at-interval '15 min',ends_at+interval '30 min'
